...
export namespace FxOrmQueries {
   ...
   */
  export type WhereObjectInput = null | undefined | Fibjs.AnyObject

  export interface OperatorFunction<T_OPNAME = string> {
    (value?: any): {
  ...
  export type OperatorFunctionResult = FxOrmTypeHelpers.ReturnType<OperatorFunction>

  export type OPERATOR_TYPE_CONJUNCTION = 'and' | 'or' | 'xor'
  export type OPERATOR_TYPE_ASSERT = 'not' | 'is'
  export type OPERATOR_TYPE_PREDICATE = 'between'
  export type OPERATOR_TYPE_COMPARISON = 'eq' | 'ne' | 'gt' | 'gte' | 'lt' | 'lte' | 'like'
   ...
   * ```
   */
  export interface HqLNormalizer {
    collection: string
  ...
  export declare class Class_QueryBuilder<T_RETURN = any> {
    ...
    findByRef<T = any>(
      refName: string,
      complexWhere: FxOrmInstance.Class_Instance | FxOrmTypeHelpers.FirstParameter<FxOrmQueries.Class_QueryBuilder['find']>['where'],
    ...
    parseHQL<T = any>(
      hql: FxOrmTypeHelpers.Parameters<typeof Class_ORM['parseHQL']>[0]
    ): ReturnType<typeof Class_ORM['parseHQL']>
  }
}
