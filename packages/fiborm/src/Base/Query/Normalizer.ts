...
import hql = require('@fiborm/orm-hql')
...
export default function getNormalizedHQLObject(
    ...args: FxOrmTypeHelpers.Parameters<typeof Class_ORM['parseHQL']>
) {
    const [sql, opts] = args
    ...
    if (models && !isEmptyPlainObject(models)) {
        tobj.collection = json.sourceTables.find((table: string) => models.hasOwnProperty(table)) || null
        if (!tobj.collection)
    ...
    make__selectableFields: {
        if (json.parsed.selection.type === 'select_all') {
             ...
             */
            json.parsed.selection.columns.forEach((selectCol: any) => {
