...
import { FxOrmDML } from "./DML";
import { FxOrmInstance } from "./instance";
import { FxOrmModel } from "./model";
import { Class_ORM } from "./ORM";
import { FxOrmTypeHelpers } from "./_common";
export declare namespace FxOrmQueries {
    /**
    ...
    class Class_QueryBuilder<T_RETURN = any> {
         ...
         */
        find(opts?: Omit<FxOrmTypeHelpers.SecondParameter<FxOrmDML.DMLDialect['find']>, 'connection'> & {
            return_raw?: boolean;
        }): T_RETURN[];
        findByRef<T = any>(refName: string, complexWhere: FxOrmInstance.Class_Instance | FxOrmTypeHelpers.FirstParameter<FxOrmQueries.Class_QueryBuilder['find']>['where'], mergeModelFindOptions?: FxOrmTypeHelpers.FirstParameter<FxOrmQueries.Class_QueryBuilder['find']>): T[];
        /**
         * @description get first tuple from remote endpoints
         */
        one(opts?: FxOrmTypeHelpers.FirstParameter<FxOrmQueries.Class_QueryBuilder['find']>): T_RETURN;
        /**
         * @description get first tuple from remote endpoints
         */
        get(id?: string | number | (string | number)[] | FxOrmTypeHelpers.FirstParameter<FxOrmQueries.Class_QueryBuilder['find']>['where']): T_RETURN;
        /**
         ...
         */
        count(opts?: Omit<FxOrmTypeHelpers.SecondParameter<FxOrmDML.DMLDialect['count']>, 'connection'>): number;
        first(): T_RETURN;
        last(): T_RETURN;
        all(): T_RETURN[];
        parseHQL<T = any>(hql: FxOrmTypeHelpers.Parameters<typeof Class_ORM['parseHQL']>[0]): ReturnType<typeof Class_ORM['parseHQL']>;
    }
}
