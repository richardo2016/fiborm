...
export const getCollectionPropertiesSync: FibOrmSqlDDLSync__Dialect.Dialect['getCollectionPropertiesSync'] = function (
	...
	let columns = <{ [col: string]: FibOrmSqlDDLSync__Column.PropertySQLite }>{}, m;

	for (let i = 0; i < cols.length; i++) {
		const column = <FibOrmSqlDDLSync__Column.PropertySQLite>{};
		const dCol = cols[i];
...
export const hasCollectionColumnsSync: FibOrmSqlDDLSync__Dialect.Dialect['hasCollectionColumnsSync'] = function (
	...
	const cols = getCollectionColumnsSync<FibOrmSqlDDLSync__Column.ColumnInfo__SQLite>(dbdriver, name)
...
export const processKeys: FibOrmSqlDDLSync__Dialect.Dialect['processKeys'] = function (keys) {
	if (keys.length === 1) {
		return [];
	}
...
export const supportsType: FibOrmSqlDDLSync__Dialect.Dialect['supportsType'] = function (type) {
	switch (type) {
		case "boolean":
		case "enum":
			return "number";
...
export const getType: FibOrmSqlDDLSync__Dialect.Dialect['getType'] = function (
	collection, property: FibOrmSqlDDLSync__Column.PropertySQLite, driver, opts
) {
	const { for: _for = 'create_table' } = opts || {}

	let type: false | FibOrmSqlDDLSync__Column.ColumnType_SQLite = false;
	let customType = null;
...
function convertIndexRows(
	rows: FibOrmSqlDDLSync__Driver.DbIndexInfo_SQLite[]
): {[k: string]: FibOrmSqlDDLSync__Driver.DbIndexInfo_SQLite} {
	const indexes = <{[k: string]: FibOrmSqlDDLSync__Driver.DbIndexInfo_SQLite}>{};
